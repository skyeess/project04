...
def sign_in():
    ...
    if result:
        ...
        response.set_cookie("mytoken", token)  # Set the token as a cookie
        return response
    else:
        return jsonify(
            {
...
def posting():
    ...
    try:
        ...
        return jsonify({"result": "success", "msg": "Your post has been created"})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
...
def update_like():
    ...
    try:
        ...
        post_id_receive = request.form.get('post_give_id')
        ...
        count_heart = db.likes.count_documents({"post_id": post_id_receive, "type": type_receive})

        return jsonify({"result": "success", "msg": "Like count updated", "count_heart": count_heart})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
...
def update_star():
    ...
    try:
        ...
        count_star = db.likes.count_documents({"post_id": post_id_receive, "type": type_receive})

        return jsonify({"result": "success", "msg": "Star count updated", "count_star": count_star})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
...
def update_thumbs():
    ...
    try:
        ...
        if action_receive == 'thumbs':
            db.likes.insert_one(doc)
        else:
            db.likes.delete_one(doc)

        count_thumbs = db.likes.count_documents({"post_id": post_id_receive, "type": type_receive})

        return jsonify({"result": "success", "msg": "thumbs count updated", "count_thumbs": count_thumbs})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
